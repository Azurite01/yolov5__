...
1. Before training, carefully check whether your format meets the requirements. The library requires the data set format to be in VOC format, and the required content includes input images and labels.
   ...
   2. The size of the loss value is used to judge whether it is convergent. What is more important is that there is a convergence trend, that is, the loss of the verification set continues to decrease. If the loss of the verification set basically does not change, the model has basically converged.
...
if __name__ == "__main__":
    ...
    #Be sure to modify classes_path before training so that it corresponds to your own data set
    # ---------------------------------------------------------------------#
    classes_path = r"model_data/my_classes.txt"
    # ---------------------------------------------------------------------#
    #   anchors_path    代表先验框对应的txt文件，一般不修改。
    #   anchors_mask    用于帮助代码找到对应的先验框，一般不修改。
    #anchors_path represents the txt file corresponding to the a priori frame, which is generally not modified.
    # anchors_mask is used to help the code find the corresponding a priori frame and is generally not modified.
    # ---------------------------------------------------------------------#
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = r"model_data/yolov5_n_v6.1.pth"
    # ------------------------------------------------------#
    ...
    # If model_path is not set, pretrained = False, Freeze_Train = Fasle, training starts from 0, and there is no process of freezing the trunk.                   如果不设置model_path，pretrained = False，Freeze_Train = Fasle，此时从0开始训练，且没有冻结主干的过程。
    ...
    #   创建yolo模型
    # ------------------------------------------------------#
    model = YoloBody(anchors_mask, num_classes, phi, pretrained=pretrained, at=at)
    if not pretrained:
        weights_init(model)
    if model_path != r"model_data/yolov5_n_v6.1.pth":
        # ------------------------------------------------------#
        #   权值文件请看README，百度网盘下载
        # ------------------------------------------------------#
        if local_rank == 0:
